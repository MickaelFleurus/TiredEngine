...
namespace Component {
CSpriteComponent::CSpriteComponent(Core::CGameObject& owner, CComponentManager& componentManager)
...
void CSpriteComponent::updatePosition() {
    ...
    //     if (std::holds_alternative<SpriteIDWrapper>(*mSpriteId)) {
    //         mWindow.moveSprite(std::get<SpriteIDWrapper>(*mSpriteId).id, position.x, position.y);
    //     } else if (std::holds_alternative<TextIDWrapper>(*mSpriteId)) {
    //         mWindow.moveText(std::get<TextIDWrapper>(*mSpriteId).id, position.x, position.y);
    //     }
    // }
}
